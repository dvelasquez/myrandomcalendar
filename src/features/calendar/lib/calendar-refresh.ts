...
export interface CalendarRefreshOptions {
  startDate?: Date;
  endDate?: Date;
  onSuccess?: (data: CalendarPageData) => void;
...
export async function refreshCalendarData(
  ...
  try {
    ...
    const result = await actions.calendar.getCalendarPageData(formData);
    ...
    const errorMessage =
      (error as Error).message || 'An error occurred while refreshing calendar';
    ...
    button.addEventListener('click', async () => {
      ...
      try {
        await refreshCalendarData({
          ...options,
          onSuccess: data => {
            jsonOutput.textContent = JSON.stringify(data.calendarData, null, 2);

            // Update events count display if it exists
            const eventsCountElement = document.querySelector(
              'p:has(strong:contains("Events Found"))'
            );
            if (eventsCountElement && data.calendarData?.events) {
              eventsCountElement.textContent = `Events Found: ${data.calendarData.events.length}`;
            }
          },
          onError: error => {
